    ...
    { 
       ...
       vector<vector<int>> dp(n+1,vector<int>(W+1,0));
        for(int i = 1;i<n+1;i++)
        {
            ...
            {
                /*
                * if weight of current coin is lesser than the weight of curent knamsack
                * i.e.{
                *   wt[i-1]<=j
                * }
                * we have wo options, either to include it, or to exclude it
                * to exclude it 
                * {
                *   dp[i][j] = dp[i-1][j]
                * }
                * to include it{
                *   dp[i][j] = value of current coin + value to knapsack if the weight of 
                *   knapsack was j-wt[i-1] and we didn't have this coin
                * } i.e.{
